...
The transport business operates as both Jay Ambey Road Line and Shree Maruti Transport Service, with a shared team, owned tanker fleet and pan-India service network. Present both names equally without suggesting a parent company or subsidiary. Use the description in `design.md` consistently across the website. The Jay Ambey page uses the supplied JARL logo and the same fleet, services and contact details. It is linked from the header, About page and footer at `/jay-ambey-road-line`.

...
Office location map, external Google Maps directions link, and a direct email link with a mail icon in the footer. The enquiry form sends to the confirmed logistics address from the website through the server endpoint. If delivery fails, offer an email-app draft and copy option. Only confirm submission after the SMTP server accepts it. Add a direct phone link when supplied or verified.
